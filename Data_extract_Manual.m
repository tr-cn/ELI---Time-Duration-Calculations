...
if true
    % clear; close all; clc;
     img = imread('OAP_Zoomed_spectrum_laptop.jpeg');
    ...
    disp(['Y-Values (pixels): ', num2str(y_pixels')]); % װגט��� װבױה��� �װ�ױ �׀׳ט ��ױ� Y
...
hold on;
plot([fwhm_left_indx fwhm_right_indx], [I_half_fs I_half_fs], 'b--', 'LineWidth', 1.5); % חױ FWHM
title('Time-Limited Pulse Intensity Profile');
xlabel('Time (fs)');
ylabel('Normalized Intensity');
legend('Pulse Intensity', ['FWHM = ' num2str(fwhm_fs, '%.2f') ' fs'], 'Location', 'best');
xlim([-10*fwhm_fs, 10*fwhm_fs]); % װׁׂ�ך װז�ט �״ױױ׳ ט�ױױא״�
grid on;
